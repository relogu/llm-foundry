...
class HasGetParamGroups(Protocol):
    ...
    This is useful for models that have custom parameter groups or need to
    modify the optimizer configuration based on their parameters.
    Such as models using MuP or CompleteP.
    """

    # NOTE: This is a protocol, so it can be used with isinstance checks.
    # NOTE: The method signature must match the one in the model.

    def get_optimizer_param_groups(  # <- exact signature
        self,
        optimizer_config: dict[str, Any],
    ) -> tuple[Iterable[torch.Tensor] | Iterable[dict[str, Any]], dict[str, Any]]:
        """Get the optimizer parameter groups and any modified optimizer config.

        Parameters:
        -----------
            optimizer_config (dict[str, Any]): The optimizer configuration.

        Returns:
        --------
            tuple[Iterable[torch.Tensor] | Iterable[dict[str, Any]], dict[str,
            Any]]: A tuple containing the parameter groups and any modified
...
def build_optimizer(
    ...
    # NOTE: Modified from Composer's build_optimizer to support models that
    # have custom parameter groups or need to modify the optimizer configuration
    # based on their parameters. Such as models using MuP or CompleteP.
    if isinstance(model, HasGetParamGroups):
        # NOTE: call to make sure disable_grad is correctly applied
        # as well as any other transformations
        # NOTE: incompatible with manually specifying
        # param groups in optimizer_config
        tmp_param_groups = _extract_param_groups(model, optimizer_config)
        if tmp_param_groups != model.parameters():
            raise ValueError(
                'The model has custom parameter groups, but the optimizer config '
                +
                'does not support this. Please remove the `param_groups` key from '
...
def build_tokenizer(
    ...
    signal_file_path = (
        dist.get_node_signal_file_name() + f"_wr_{dist.get_node_rank()!s}"
    )
...
def build_icl_evaluators(
    ...
    for icl_cfg in icl_tasks_list:
        ...
        for num_fewshot in list(icl_cfg['num_fewshot']):
            ...
            if (
                'has_categories' in icl_cfg and icl_cfg['has_categories'] and
                isinstance(dataloaders, dict)
            ):
